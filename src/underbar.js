(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    let mapped = [];
    _.each(collection, element => mapped.push(iterator(element)));
    return mapped;
  };
  ...
  _.extend = function(obj) {
    _.each(arguments, function(newProp) { // -> for each called arguments, invoke function with newProp
      for (let key in newProp) { // -> run for loop over newProp
        obj[key] = newProp[key]; // -> assign obj[key] to newProp[key]
      }
    });
  ...
  _.defaults = function(obj) {
    _.each(arguments, function(newProp) { // -> for each called arguments, invoke function with newProp
      for (let key in newProp) { // -> run for loop over newProp
        if (obj[key] === undefined) { // -> if obj[key] doesn't exist (undefined)
          obj[key] = newProp[key]; // -> assign obj[key] to newProp[key]
        }
      }
    });
  ...
  _.memoize = function(func) {
    ...
    if (called[key] !== undefined) {
      ...
      called[key] = func.apply(this, arguments);
      return called[key];
      }
    }
  };
